...
# Logs sent to file "IRCLogs.txt" / Rewrites log file with every run
import socket, codecs, sys, traceback
...
print('Welcome to Muffinlicious\' Python Freenode IRC Client!')
IRCLog = open(logfile, 'w')
IRCLog.write('Welcome to Muffinlicious\' Python IRC Logs!\r\n')
...
topic = False
def client(text):
    global ident, join, topic, status
    sep = text.split()
    if find(text, '!'):
...
MuffinBot\'s slap function', True)
        ...
        spam += 2
    elif find(text, ':!challenge'):
        opponent = text[1:][text[1:].index(' :') + 2:].split()[1]
        irc.send('PRIVMSG %s :
    elif find(text, ':!urban'):
        try:
            queue = text[1:][text[1:].index(' :') + 2:].split()[1]
        except IndexError:
            pass
        else:
            ...
            content = parse[:parse.index('</div>')].replace('\r', ' ').split('\n')
